...
Regice::Regice(const JsonObject& obj) : details(obj) {
...
void Regice::GetNearestEntity(std::vector<IEntity*> scheduler) {
...
void Regice::Update(double dt, std::vector<IEntity*> scheduler) {
  if (available) {
...
void Regice::Rotate(double angle) {
  Vector3 dirTmp = direction;
  direction.x = dirTmp.x * std::cos(angle) - dirTmp.z * std::sin(angle);
  direction.z = dirTmp.x * std::sin(angle) + dirTmp.z * std::cos(angle);
}

void Regice::Jump(double height) {
  if (goUp) {
    position.y += height;
    jumpHeight += height;
    if (jumpHeight > 5) {
...
void Regice::SetPosition(Vector3 pos_) {
